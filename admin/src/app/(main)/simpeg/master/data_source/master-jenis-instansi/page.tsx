...
const Page = () => {
    ...
    const visibleList = filteredList.slice((pageSelect - 1) * pageLimit, pageSelect * pageLimit)

    const deleteMutation = useMutation({
        mutationFn: (id: string) => deleteData(
            `${url}/api/v1/simpeg/master/ref_jenis_instansi/delete/${id}`
        ),
        onSuccess: () => {
            queryclient.invalidateQueries({ queryKey: ["ref_jenis_instansi"] })
        },
        onError: (err) => {
            alert(`Error : ${err}`)
        }
    ...
    return (
        <div>
            <TextSeparate title='Master Jenis Instansi' />
                        ...
                        <div className='flex gap-1 relative'>
                            ...
                            <div className='w-12'>
                                <BButton
                                    ...
                                    size='md'
                                    onClick={() => {
                                        setForm({ id: '', kode: '', nama: '', created_by: 'user.id' })
                                        setCreateType(false)
                                        setModalCreate(true)
                                    }}
                                >
